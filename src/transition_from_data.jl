...
function make_grid_min_max(logx::AbstractVector; n::Int=31, scal::Real=5.0)
    a,b = extrema(logx)
    a -= log(scal)
    b += log(scal)
    step = (b-a)/(n-1)
...
function BMtransition_from_data(logxy::Array{T,2}, matching::Bool; zero_cov::Bool=true, minp::Real=1e-9, kwargs...) where {T<:Real}
    0.0 <= minp < 1.0 || throw(DomainError())
    grids = ((make_grid_min_max(logxy[:,j]; kwargs...) for j in 1:size(logxy, 2))...)
    Σ = cov(diff(logxy,1))
    zero_cov  &&  Σ .= Diagonal(Σ)
    transition = markov_transition_match_option((xy::NTuple) -> [xy...], (xy::NTuple) -> Σ, Val{matching}, minp, grids...)
    return (grids, Σ, transition, )
end

function BMtransition_from_data(logx::Array{T,1}, matching::Bool; minp::Real=1e-9, kwargs...) where {T<:Real}
    0.0 <= minp < 1.0 || throw(DomainError())
    grid = make_grid_min_max(logx; kwargs...)
    σ = sqrt(var(diff(logx)))
    transition = markov_transition_match_option( (x) -> x[1], (x) -> σ, Val{matching}, minp, grid)
